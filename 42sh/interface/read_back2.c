...
/*   Updated: 2017/12/15 14:10:05 by angonyam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				main(void)
{
	...
	while (1)
	{
		...
		if (ft_isprint(c))
			test = rewrite(test, c);
		if (ft_strlen(buffer) == 3 && buffer[2] == 68 && count < ft_strlen(test))
		{
			count++;
			tputs(tgetstr("le",	NULL), 1, fputchar);
		}
		else if (ft_strlen(buffer) == 3 && buffer[2] == 67 && count > 0)
		{
			count--;
		...
		//ft_printf("buffer is %c\nit's size is %d\n", buffer[0], ft_strlen(buffer));
